# compile_perl.tcl --
#
#	This file implements perl code generation and testing
#
...
#   Returns ...
#
proc ::perl::init {version interp args} {
...
proc ::perl::interpreter {{interp ""} args} {
...
proc ::perl::widget_info {widget {wcmd {}}} {
    set cmd {
	use Tk;
	use Tk::Menu;
	use Data::Dumper;
	...
	$top = MainWindow->new();
	$w   = $top->%2s();
	for (Dumper($w->configure())) { $a = substr $_, 1, -1; print "$a\n"; }
	exit;
    }
    if {$wcmd eq ""} {
	set wcmd [namespace tail $widget]
    }
    set ecmd [format $cmd $widget $wcmd]
    variable interpreter
    return [exec $interpreter << $ecmd]
}

proc ::perl::widget_config {widget {wcmd {}}} {
    set opts [split [widget_info $widget $wcmd] \n]
    set conf [list]
    foreach optline $opts {
	# skip aliases
	set opt [split [string map {' ""} $optline] ,]
	if {[llength $opt] < 5} { continue }
	foreach {name cname dbname def cur} $opt { break }
	append conf \t\{[list option $name \
			     -reflect 0 \
			     -type     [::widget::opt_type "" "" $name] \
			     -category [::widget::opt_category "" "" $name] \
			     -default $def \
			    ]\}\n
    }
    return \{\n$conf\}
...
#   Returns ...
#
proc ::compile::perl::init {ver args} {
    # PERL SPECIFIC
...
#
proc ::compile::perl::quote {str opt} {
...
#   Returns ...
#
proc ::compile::perl::comment {txt {prenew 2} args} {
    variable indent
...
#   Returns ...
#
proc ::compile::perl::command {prefix name arglist body {desc ""} args} {
    set script ""
    append script "# $name --\n#\n"
    if {$desc != ""} {
...
#   Returns ...
#
proc ::compile::perl::option {cmd name option value args} {
    variable use_images
    variable use_vars
    variable init_vars
    ...
    # due to a, err, feature in perl/Tk, an uninitialized variable
    # for a scale widget brings warning with perl -w, so initialize it.
    set isScaleVar [expr {$cmd eq "Tk scale" && $option eq "-variable"}]
    foreach {match var} [regexp -all -inline {\\(\$\w+)} $value] {
	lappend use_vars $var
	if {$isScaleVar} {
	    lappend init_vars "$var = 0;"
	}
    }
...
#   Returns ...
#
proc ::compile::perl::require {type w args} {
...
#   Returns ...
#
proc ::compile::perl::targetHeader {file uifile prefix args} {
...
#   Returns ...
#
proc ::compile::perl::targetFooter {prefix args} {
    return "\n1;\n"
}
...
#   Returns ...
#
proc ::compile::perl::includeHeader {file uifile prefix args} {
    set script ""
...
#   Returns ...
#
proc ::compile::perl::uiProcBegin {prefix args} {
    ...
    if {$init_vars != ""} {
	append script [join $init_vars "\n"]\n
    }
    # Get the use statement we need - should at least include Tk
    foreach grp [lsort [array names REQS]] {
	append script "use $grp;\n"
    }
    append script "use Tk::Menu;\n"
    append script "\n"
	...
	append script "${indent}\$root->Photo('$img',\
		-file => $use_images($img));\n"
    }
...
#   Returns ...
#
proc ::compile::perl::uiProcEnd {args} {
    variable indent ""
    return "\}\n"
}
...
#
proc ::compile::perl::widget {type name master options args} {
    variable indent
    variable widget_cmds
    ...
    set script ""
    append script "${indent}our(\$$name) = \$root->$wcmd\("

    # setting of options
    set in2 "\t"
    foreach {opt value} $options {
	# perl/Tk crashes on a null cursor!
	if {$opt == "-cursor" && [string length $value]==0} {
	    continue
	}
...
#   Returns ...
#
proc ::compile::perl::bindtags {type name tags args} {
    variable indent
    variable bindtags
...
#   Returns ...
#
proc ::compile::perl::postWidgets {args} {
    variable indent
    variable bindtags
    variable widget_cmds
...
#   Returns ...
#
proc ::compile::perl::menu {master type options args} {
    variable indent

    set script ""

    append script "${indent}\$$master->$type\("
    foreach {opt value} $options {
	if {$opt eq "-menu"} {
	    append script "\n\t$opt => \$$value,"
...
#   Returns ...
#
proc ::compile::perl::attachmenu {name args} {
    variable indent
...
#   Returns ...
#
proc ::compile::perl::geometry {name master row column options args} {
    variable indent
...
#   Returns ...
#
proc ::compile::perl::resizing {master dim weights minsizes pads args} {
    variable indent
    ...
    set index 0
    set dim   [string totitle $dim]
    foreach weight $weights \
	    size   $minsizes \
	    pad    $pads {
	# original perl gen limits weight to max 1.
	append script "${indent}$master->grid${dim}configure([incr index],\
...
proc ::compile::perl::translate {name} {
...
#   Returns ...
#
proc ::compile::perl::test {name {validate ""} args} {
    set exe $::perl::interpreter
    if {![file exists $exe]} {
	if {$exe == ""} {
	    status_message "No known perl interpreter"
	} else {
...
#   Returns ...
#
proc ::compile::perl::kill_test {args} {
    if {[info exists ::perl::fid]} {
	catch {kill [pid $::perl::fid]}
	catch {close $::perl::fid}
	unset ::perl::fid
    }
    disable_kill_test
}
...
#   Returns ...
#
proc ::compile::perl::isTestRunning {args} {
    return [info exists ::perl::fid]
}
...
#  code
#
proc ::compile::perl::parseCallbacks {code} {
    variable CMDS
...
#
proc ::compile::perl::parseInclude {prefix file} {
...
#
proc ::compile::perl::compileInclude {
